...
type ActiveTab = 'gemini' | 'huggingface';
...
const SettingsModal: React.FC<SettingsModalProps> = ({ onClose }) => {
  ...
  const tabButtonClasses = (tabName: ActiveTab) =>
    `px-4 py-2 text-sm font-medium rounded-md focus:outline-none focus:ring-2 focus:ring-pink-500 transition-colors duration-200 w-1/2 ${
      activeTab === tabName
      ...
      >
        ...
        <div className="border-b border-slate-200 dark:border-slate-700 mb-6">
            <div className="flex space-x-2 p-1 bg-slate-100 dark:bg-slate-900 rounded-lg">
                <button className={tabButtonClasses('gemini')} onClick={() => setActiveTab('gemini')}>Google Gemini</button>
